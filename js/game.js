...
(function(win) {
	...
	var gb = function(option) {
		...
		var self = this,
			canObj = document.getElementById("cc"),//棋盘dom
		...
		my.playScoreA = 0;//玩家一的分数
		my.playScoreB = 0;//玩家二的分数
		//x，y 正方形数量（20*20）
		my.rectNum = my.xlen;
		 ...
		 */
		self.back = function() {
			// console.log( my.downChessmanStackC2d);
			...
			if (!my.gameover && my.downChessmanStackC2d.length > 0) {
				...
				for (var i = 0; i < backtime; i++) {
					...
					my.gobackArr.push(my.goBackC2d)//保存悔棋的步骤
		...
		//撤回悔棋,按照步骤
		self.cancelBack = () =>{
			let i =  my.gobackArr.length - 1;
			if (!my.gameover && my.gobackArr.length > 0){
				var goback =  my.gobackArr[i]
				logic({
					s:'click',
					x:goback.IX,
					y:goback.JY
